...
from src.models.clip.prompt_learner import cfgc, load_clip_to_cpu, PromptLearner
from src.utils.class_names import cddb_classnames
    ...
    def generate_prompts_from_input(self, object_labels):
        ...
        if self.topk_classes > 0:
            ...
            # Topk object label to text
                ...
                # labels: tuple of topk labels for the image
                prompts = [
                    f"a {type_image} photo of a {label}."
                    for type_image in cddb_classnames.values()
                    for label in labels[:self.topk_classes]
                ]
                self.make_prompts(prompts)
