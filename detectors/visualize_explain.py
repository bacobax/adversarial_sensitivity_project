...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--method', type=str, required=False, help='Explainability method')
...
def main():
    ...
    cache_dir = os.path.join(CACHE_ROOT, args.detector, method)
    os.makedirs(os.path.join(args.output, method), exist_ok=True)
    ...
    for batch_paths in tqdm(images):
        ...
        if idx_to_compute or args.recompute_logits:
            ...
            # for p, l in zip(batch_paths, torch.sigmoid(logits_all)):
            #     print(f'{l.item():.4f}\t{p}')
            # print()
        ...
        if idx_to_compute:
            ...
            cams_sub = detector.explain(
                sub_batch, method=args.method, class_idx=args.class_idx, batch_size=bs,
            )
        ...
        for base, folder_data in grouped_images.items():
            ...
            if args.attack is not None:
                for cat in categories:  # fixme
                    ...
                    # vulnerability = cam_orig_norm + cam_diff #.abs()
                    vulnerability = cam_diff
                    ...
                    # sigm = torch.sigmoid(folder_data[('b-free', cat)]['logit']).item()
                    # over *= sigm
