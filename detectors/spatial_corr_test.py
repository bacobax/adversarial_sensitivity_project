...
cvs_header = 'detector,attack,category,image,logit,prob,ap,mim'
        ...
        if needs_logits:
            ...
            with torch.inference_mode():
                if pd.isna(df.loc[df_key_orig, "logit"]):
                    ...
                    _set_cell(df, df_key_orig, "prob", float(sigmoid_orig))
                ...
                if pd.isna(df.loc[df_key_adv, "logit"]):
                    ...
                    _set_cell(df, df_key_adv, "prob", float(sigmoid_adv))
