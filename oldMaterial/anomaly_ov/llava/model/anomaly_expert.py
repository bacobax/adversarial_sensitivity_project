...
class AnomalyOV(nn.Module):
    def get_upsampled_anomaly_map(self, sig_multi_level_features, ov_base_features, output_size=(224, 224)):
        """
        Returns an upsampled anomaly map matching the original image size.
        Args:
            sig_multi_level_features: list of tensors, each [batch_size * num_patches, 729, self.siglip_hidden_dim]
            ov_base_features: tensor [batch_size * num_patches, 729, self.ov_token_hidden_size]
            output_size: tuple, desired output size (H, W)
        Returns:
            upsampled_map: [batch_size * num_patches, 1, H, W]
        """
        anomaly_map, _ = self.get_anomaly_map(sig_multi_level_features, ov_base_features)  # [N, 1, 27, 27]
        upsampled_map = F.interpolate(anomaly_map, size=output_size, mode='bilinear', align_corners=False)
    ...
    def load_zero_shot_weights(self, path='./pretrained_expert_7b.pth',device='cpu'):
        # Ensure checkpoints saved with CUDA tensors can be loaded on CPU-only environments
    ...
    def forward(self, ov_image_features, sig_multi_level_features, split_sizes, return_anomaly_map=False, anomaly_map_size=(224, 224)):
        ...
        # Ensure CPU execution to avoid MPS pooling limitations; restore device/dtype afterwards
        orig_device = ov_image_features.device
        orig_dtype = ov_image_features.dtype
        ov_image_features = ov_image_features.to(dtype=torch.float32)
        sig_multi_level_features = [f.to(dtype=torch.float32) for f in sig_multi_level_features]
        ...
        # Workaround for MPS adaptive pooling limitation (input sizes must be divisible by output sizes)
        # Temporarily move to CPU for pooling operations when on MPS
        pooling_device = anomaly_map.device
        if pooling_device.type == 'mps':
            anomaly_map_pool = anomaly_map.cpu()
            scaled_final_features_pool = scaled_final_features.cpu()
            in_patch_significance = self.global_pooling(anomaly_map_pool).to(pooling_device) # [batch_size * num_patches, 1, 2, 2]
            sig_tokens = (self.global_pooling(scaled_final_features_pool) / self.global_pooling(anomaly_map_pool)).to(pooling_device) # [batch_size * num_patches, self.ov_token_hidden_size, 2, 2]
        else:
        ...
        final_attention_output = final_attention_output.to(orig_device, dtype=orig_dtype)
        final_prediction = final_prediction.to(orig_device, dtype=orig_dtype)

        if return_anomaly_map:
            # Return upsampled anomaly map for each image in the batch
            upsampled_map = F.interpolate(anomaly_map, size=anomaly_map_size, mode='bilinear', align_corners=False)
            upsampled_map = upsampled_map.to(orig_device, dtype=orig_dtype)
            return major_attention_output, final_attention_output, final_prediction, upsampled_map
